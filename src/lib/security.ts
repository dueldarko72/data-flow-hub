...
export const phoneSchema = z
  ...
  .pipe(z.string().regex(/^(?:0\d{9}|\+233\d{9}|233\d{9})$/, "Enter a valid Ghana phone number"));
...
export const passcodeSchema = z.string().regex(/^\d{4}$/, "Passcode must be exactly 4 digits");
...
export async function verifyPassword(emailRaw: string, password: string): Promise<Credential> {
  ...
  const ok = !!cred && !!cred.hash && (await verifySecret(password, cred.salt, cred.hash));
  if (!ok) {
...
export async function verifyPhoneIdentity(name: string, phone: string): Promise<Credential> {
  ...
  const ok =
    !!cred && (await verifySecret(identityKey(cleanName, id), cred.phoneSalt, cred.phoneHash));
  if (!ok) {
