...
import { formatGHS, loadOrders } from "@/lib/mock-data";
import {
  availableBalance,
  loadSettings,
  loadWithdrawals,
...
function AdminWithdraw() {
  ...
  useEffect(() => {
    ...
    setDestination(loadSettings().momoNumber ?? "");
  }, [adminId]);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="glass border-0 overflow-hidden">
        <div className="border-b border-border/50 p-4 text-sm font-semibold">Withdrawal history</div>
        {history.length === 0 ? (
