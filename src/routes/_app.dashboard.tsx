...
function Dashboard() {
  ...
  useEffect(() => {
    async function init() {
      ...
      const userNotifs = await loadNotifications(user?.id);
    ...
    const channel = supabase
      .channel(`customer-dashboard-realtime-${user?.id || "guest"}`)
    ...
    <div className="space-y-6">
      ...
      <div className="hero-bg glass overflow-hidden rounded-3xl p-6 md:p-8">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <Button
            asChild
            size="lg"
            className="gradient-gold text-primary-foreground hover:opacity-90 glow"
          >
            <Link to="/buy">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          ...
          value={formatGHS(user?.balance ?? 5000)}
          icon={Wallet}
          sub="Live Supabase balance"
        />
        <StatCard
          label="Total spent"
          value={formatGHS(totalSpent)}
          icon={TrendingUp}
